  ...
  const handleProjectChange = (newIndex) => {
    ...
    tl.to(items, { opacity: 0, duration: 0.35, stagger: 0.06 })
      ...
      .fromTo(
        items,
        { opacity: 0 },
        { opacity: 1, duration: 0.35, stagger: 0.06, immediateRender: false },
        "+=0.05",
      )
        ...
        <div className="flex h-full">
          ...
          <div className="border-r border-[#1C1C21]">
            {projectData.projects.map((project, index) => (
              <div
                key={project.id}
                className={`border-b p-4 cursor-pointer transition-colors duration-200 ${
                  activeProject === index ? "bg-gray-100" : "hover:bg-gray-50"
                }`}
              ...
              >
                <Image
                  ...
                  height={75}
                  alt={project.logoAlt}
                  className={`rounded-sm transition-all duration-300 select-none ${
                    activeProject === index
            ...
            <div>
              <div ref={topRef} className="grid lg:grid-cols-3 grid-cols-1">
                <div className="p-4 border-[#1C1C21] lg:border-r border-b lg:border-b-0">
                  <ScrambleText
                    as="h4"
                    className="font-pixel-circle font-semibold text-2xl uppercase"
                ...
                <div className="p-4 border-[#1C1C21]  lg:col-span-2 self-center">
                  {currentProject.languages.map((language, index) => (
                    <span key={`${language}-${index}`}>
                      <ScrambleText
                        className="font-pixel-circle font-semibold text-xl uppercase "
                        text={language}
                      />
                      {index !== currentProject.languages.length - 1 ? (
                        <span className="font-pixel-circle text-xl uppercase ">
                          {" "}
                          {" / "}{" "}
                        </span>
                      ) : null}
              ...
              <div className="border-[#1C1C21] border-t ">
                <div className="grid lg:grid-cols-2 grid-cols-1 max-h-[1020px]">
                  <div
                    ref={leftRef}
                    className="border-[#1C1C21] lg:border-r overflow-auto order-2 lg:order-1 flex flex-col"
                  >
                    ...
                    <div
                      className={`border-[#1C1C21] text-center mt-auto ${currentProject.url && currentProject.githubURL ? "grid grid-cols-1 lg:grid-cols-2" : "block"}`}
                    >
                      {currentProject.url && (
                        <Link href={currentProject.url} target="_blank">
                          <div
                            className={`p-4 border-[#1C1C21] border-t flex flex-row gap-2 font-pixel-circle font-semibold justify-center items-center ${currentProject.url && currentProject.githubURL ? "lg:border-r lg:border-b-0" : ""}`}
                          >
                      ...
                      {currentProject.githubURL && (
                        <Link href={currentProject.githubURL} target="_blank">
                          <div
                            className={`p-4 border-[#1C1C21] border-t flex flex-row gap-2 font-pixel-circle font-semibold justify-center items-center ${currentProject.url && currentProject.githubURL ? "" : ""}`}
                          >
                            {currentProject.githubURLTitle}{" "}
                            <ArrowUpRight width={20} height={20} />
                          </div>
                        </Link>
                      )}
                  ...
                  <div
                    ref={rightRef}
                    className="p-4 select-none order-1 lg:order-2"
                  >
                    <Image
